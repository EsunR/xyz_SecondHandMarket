$(function () {
  ...
  var url = window.location.href.split('#')[0];
	...
	/*
	 * //切换面板变换url $('#info_tab').click(function(){ window.location.href = url +
	 * '#info'; }) $('#publish_tab').click(function(){ window.location.href =
	 * url + '#publish'; }) $('#want_tab').click(function(){
	 * window.location.href = url + '#want'; })
	 * $('#collection_tab').click(function(){ window.location.href = url +
	 * '#collection'; }) $('#userMsg_tab').click(function(){
	 * window.location.href = url + '#userMsg'; })
	 * $('#sysMsg_tab').click(function(){ window.location.href = url +
	 * '#sysMsg'; })
	 */
  ...
  function hashChange() {
    ...
    let hash = window.location.hash;
		/*
		 * switch(hash){ case '#info': console.log('info'); break; case
		 * '#publish': console.log('publish'); break; case '#want_tab':
		 * console.log('want_tab'); break; case '#collection_tab':
		 * console.log('collection_tab'); break; }
		 */
  }
  ...
  $('#change').click(
    function () {
      if (show_submit_change == true) {
        ...
        $('#submit_change').removeAttr('disabled').removeClass(
  ...
  $('.seller_info').click(
    function () {
      ...
      // Ajax返回的对象
      let seller_info = {
        sellerName: $(this).parents('.item_card').attr(
          'item_sellerName'),
        sellerContactWay: $(this).parents('.item_card').attr(
          'item_contactWay'),
        sellerContact: $(this).parents('.item_card').attr(
          'item_contact')
      }
      // Ajax接收数据成功后执行的函数
